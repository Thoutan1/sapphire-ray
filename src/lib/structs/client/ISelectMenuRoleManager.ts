...
export class ISelectMenuRole {
	 ...
	 */
	public createRole({ description, label, emoji, role }: MenuObject): MenuObject {
		this.roles.push({
		...
		return { description: description, label: label, emoji: emoji, role: role };
	}
	...
	public async createMenus({ content, role, channelId, type, minimum, maximum }: CreateMenuObject): Promise<Message> {
		...
		role.forEach((menuObject: MenuObject) => {
			...
			menuOptions.push({
				...
				description: `Click untuk mendapatkan ${getRole(channel, menuObject)?.name} `.substring(0, 50)
			});
...
function getRole(channel: TextChannel, menuObject: MenuObject) {
	return channel.guild.roles.cache.get(menuObject.role);
}
