import { Message, MessageButton, MessageActionRow, TextChannel } from 'discord.js';
...
export class IButton {
	 ...
	 */
	public createCompo({ color, label, emoji, role }: ButtonsObject): ButtonsObject {
		this.roles.push({
			color: color,
			label: label,
			emoji: emoji,
	 ...
	 */
	public async createButtonsComponent({ content, role, channelId }: CreateButtonObject): Promise<Message> {
